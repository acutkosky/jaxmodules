...
@dataclass(frozen=True)
class WarpSpecializedForwardConfig:
    """Compile-time resource allocation for a warp-specialized forward."""
...
def _warp_specialized_dq_config_for_head_dim(
    ...
    if 144 <= head_dim <= 256 and head_dim % 16 == 0:
        return WarpSpecializedDQConfig(
            block_q=64,
            block_kv=64,
            num_compute_wgs=1,
...
def _warp_specialized_dkv_config_for_head_dim(
    ...
    if 80 <= head_dim <= 256 and head_dim % 16 == 0:
...
def _prefer_generated_warp_specialized_backward(
    head_dim: int,
    *,
    is_causal: bool,
) -> bool:
    """Whether generated 64-row kernels beat the generic tiled backward."""

    if not 80 <= head_dim <= 240 or head_dim % 16:
        return False
    # At D=192 the causal kernel crosses a register-residency boundary while
    # the generic 64-row kernel remains efficient. The full unmasked traversal
    # amortizes that cliff and still favors producer/consumer staging.
...
def _select_warp_specialized_forward_config(
    ...
    *,
    ...
    """Select a statically generated producer/consumer kernel configuration."""
    ...
    *,
    ...
    def kernel(
        ...
        def process_tile(kv_step, carry, mask):
            ...
            probabilities = jnp.exp2(scores * log2e - safe_next_row_max_broadcast)
            if mask is not None:
        ...
        if is_unmasked and not is_causal:
            ...
            carry = lax.fori_loop(
                0,
                full_kv_stop,
                (unmasked_kv_body if is_unmasked else user_masked_kv_body),
                carry,
            )
            output_accumulator, row_max, row_sum = lax.fori_loop(
                full_kv_stop,
...
def _mosaic_attention_forward_warp_specialized(
    ...
    *,
    is_causal: bool = False,
    _config: WarpSpecializedForwardConfig | None = None,
) -> tuple[jax.Array, jax.Array]:
    ...
    if config is None:
        raise ValueError(
            f"no warp-specialized configuration is available for D={query.shape[3]}"
        )
    ...
    compute_registers = config.compute_registers
...
def _mosaic_attention_backward_warp_specialized(
    ...
    *,
    ...
    def load_shared_rhs_without_transpose(_, smem_ref):
        ...
        matrix_shape = ir.Attribute.parse("#nvvm.ld_st_matrix_shape<m=8, n=8>")
    ...
    def state_body(state):
        ...
        def kernel_with_scratch(
            ...
            def compute_warpgroup():
                ...
                query_slice = pl.ds(query_base, block_q)
                matrix_smem = (
                    transpose_smem.at[wg_index] if compute_kv_gradients else None
                )
                ...
                def process_kv_tile(kv_step, query_gradient):
                    ...
                    plgpu.barrier_wait(key_for_scores_ready.at[key_slot])
                    key_for_scores = load_shared_rhs(key_for_scores_smem.at[key_slot])
                    scores = plgpu.mma(
                        plgpu.layout_cast(
                            jnp.zeros(
                                (block_q, block_kv),
                    ...
                    plgpu.barrier_arrive(key_for_scores_consumed.at[key_slot])
                    query_gradient = mma_with_accumulator_lhs(
                        query_gradient,
                        ds_component,
                        key_for_dq,
                    ...
                    probability_rhs = load_shared_rhs_without_transpose(matrix_smem)
                    gradient_for_dv = load_shared_lhs(gradient_for_dv_smem.at[wg_index])
                    ...
                    query_for_dk = load_shared_lhs(query_for_dk_smem.at[wg_index])
            ...
            def memory_warpgroup():
                ...
                if compute_kv_gradients:
                    for consumer in range(num_compute_wgs):
                        ...
                        plgpu.copy_gmem_to_smem(
                            gradient_transposed_ref.at[query_transposed_slice],
                ...
                def refill_pipeline(kv_step):
                    ...
                    plgpu.barrier_wait(key_for_scores_consumed.at[key_slot])
                    plgpu.copy_gmem_to_smem(
    ...
    *,
    ...
    if gradient_kind == "both":
        requested_pipeline_depth = (
            ...
            else (2 if head_dim <= 192 else 1)
        )
    elif gradient_kind == "key":
        requested_pipeline_depth = 2 if head_dim <= 128 else 1
    else:
        requested_pipeline_depth = 2 if head_dim <= 192 else 1
    ...
    query_transposed_scratch = (
        ...
        if compute_key_gradient and stage_transposed_operands
        else unused_scratch
    )
    gradient_scratch = (
        row_operand_scratch
    ...
    gradient_transposed_scratch = (
        transposed_operand_scratch
        if compute_value_gradient and stage_transposed_operands
        else unused_scratch
    )
    barriers = tuple(
    ...
    def kernel(
        ...
        def memory_warpgroup():
            ...
            def refill_pipeline(query_offset):
                ...
                next_query_tile = query_start + query_offset + pipeline_depth
                next_query_slice = pl.ds(
                    next_query_tile * block_q,
                    block_q,
                )
