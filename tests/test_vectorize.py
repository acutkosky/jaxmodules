...
def test_multi_vmap_multiple_axes():
    ...
    result = multi_vmap(
        fn,
        in_axes=[(0, None, 1), (1, None, None)],  # Map x over first axis, z over second axis
        out_axes=(0, 1)
    )(x, y, z)
...
def test_multi_vmap_mixed_mapping():
    ...
    y = jnp.array([[4, 5, 6],          # Will be partially mapped
                    [7, 8, 9],
                    [10, 11, 12]])
